...
export const SocketProvider: React.FC<{ children: ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    newSocket.on("playerStatusUpdate", (data: { players: Player[] }) => {
      ...
      setRoomData((prev) => (prev ? { ...prev, players: data.players } : null));
    });
    ...
    newSocket.on("wordRejected", (data: { reason: string }) => {
      toast.error(`Word rejected: ${data.reason}`, { position: "top-right" });
    });

    newSocket.on("gamePaused", (data: { reason: string }) => {
      setGameState((prev) => ({ ...prev, isPlaying: false }));
      toast.info(`Game paused: ${data.reason}`, { position: "top-right" });
    });
