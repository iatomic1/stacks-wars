...
import { insertLobbySchema, lobby } from "../db/schema/lobby";
...
export const createLobby = async (input: CreateLobbyInput) => {
  const validatedInput = insertLobbySchema.parse(input);
...
export const createLobbyWithPool = async (
  ...
  const validatedInput = insertLobbySchema.parse(lobbyInput);
