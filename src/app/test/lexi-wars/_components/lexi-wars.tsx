...
export default function MultiplayerLexiWars({ oppsData }: LexiWarsProps) {
  ...
  const validateWord = (word: string) => {
    if (word.length < minWordLength) {
      toast.error(`Word must be at least ${minWordLength} characters long!`, {
        position: "top-center",
      });
      setWord("");
      return false;
  ...
  useEffect(() => {
    const loadHighScore = async () => {
      const record = await gameDb.singleplayerV1.get(1);
      if (record) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gradient-to-b from-background to-muted/30">
      <main className="flex-1">
        <div className={cn("container max-w-3xl px-4 py-4 sm:px-6 sm:py-6")}>
          <Link
            href="/games"
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <User
                        className={`h-5 w-5 ${
                          isMultiplayer ? "text-yellow-500" : "text-green-500"
                        }`}
                      />
                      <h3
          ...
          <div className="sr-only" role="alert">
            This is a competitive typing game that requires manual keyboard
            input. Screen readers are not supported for fair gameplay.
          </div>
        </div>
      </main>
    </div>
